"""
Implementation of Kernel Improved Possibilistic c-Means 2 Alternative Optimization Algorithm
"""
...
class KIPCM2:
    ...
    def compute_centroids_1(self, fuzzy_matrix, t_matrix, centroids):
        ...
        prod_assignments = np.multiply(fuzzified_assignments, typical_assignments)
    ...
    def compute_gamma_0(self, fuzzy_matrix, centriods):
        ...
        gamma = np.array(
            [self.k * np.sum(v, axis=0) / sum_fuzzified_assigments[i]
             for i, v in enumerate(centroid_numerator)]
        )
        ...
        for i in range(len(self.data)):
            ...
            for j in range(self.num_clusters):
                den = \
                    (g_matrix[j] *
                     (1 - (np.exp(-(
                             (2 - 2 * self.gaussian_kernel(centroids[j], self.data[i])) /
                             g_matrix[j])))))
    ...
    def compute_objective_function_0(self, centroids, t_matrix, fuzzy_matrix):
        ...
        objective_objective = \
            np.sum([[(2 - 2 * self.gaussian_kernel(self.data[i], centroids[j])) * t_matrix[i][j] *
    ...
    def get_centroids_1(self):
        ...
        for num in range(self.num_iter):
            ...
            centroids = self.compute_centroids_1(u_matrix, t_matrix, centroids_old)
        ...
        for i in range(len(x)):
            ...
            for j in range(self.num_clusters):
                denomenator += np.power(
                    (1 / (self.eta[0][j] *
                          (1 - np.exp(-((2 - 2 * self.gaussian_kernel(
    ...
    def predict_tipicality_(self, x):
        ...
        for i in range(len(x)):
            for j in range(self.num_clusters):
                exponent = \
                    (2 - 2 * self.gaussian_kernel(self.fit_cent[0][j], self.data[i]))\
                    / self.eta[0][j]
