"""
Implementation of Basic Graded Possibilistic alternative optimisation algorithm
"""
...
class BGPC:
    ...
    def compute_v_matrix(self, centroids, b, v_matrix):
        ...
        for i in range(len(self.data)):
            for j in range(self.num_clusters):
                exponent = np.exp(-(get_distance(centroids[j], self.data[i]) / b))
                vik_new = exponent
    ...
    def predict_tipicality(self, x):
        ...
        for i in range(len(x)):
            for j in range(self.num_clusters):
                exponent = np.exp(-(get_distance(self.fit_cent[0][j], x[i]) / self.b[0]))
                vik_new = exponent
                initial_v_matrix[i][j] = vik_new
        return initial_v_matrix
