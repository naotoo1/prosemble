"""
Implementation of Fuzzy c-Means Alternative Optimisation Algorithm
"""
...
class FCM:
    ...
    def __init__(self, data, c, m, num_iter, epsilon, ord, set_U_matrix=None, plot_steps=False):
        ...
        if self.set_U_matrix == 'kmeanss':
            self.model = kmeans_plusplus(
                data=self.data,
                c=self.num_clusters,
                num_inter=None,
                epsilon=None,
            ...
            self.model.fit()
            self.set_U_matrix = self.model.get_proba(self.data)
        ...
        for i in range(len(self.data)):
            ...
            for j in range(self.num_clusters):
                denomenator += np.power(
                    1 / get_distance(centroids[j], self.data[i]), 2 / (self.fuzzifier - 1))
            for j in range(self.num_clusters):
                uik_new = np.power(1 / get_distance(centroids[j], self.data[i]),
    ...
    def compute_objective_function(self, centroids, u_matrix):
        objective_function = np.sum(
            [[get_distance_squared(self.data[i], centroids[j]) *
    ...
    def _distance_(samples, centroid):
        return np.sum([get_distance(sample, centroid) for index, sample in enumerate(samples)])

    @staticmethod
    def _nearest_centroids(sample, centroids):
        return np.argmin([get_distance(sample, centroid) for centroid in centroids])
    ...
    def _centroid_stability(self, centroids_num, centroids):
        ...
        if self.ord is None and self.epsilon is None:
            distance = [get_distance(centroids_num[index], centroids[index]) for index in
    ...
    def get_centroids(self):
        ...
        for num in range(self.num_iter):
            ...
            obj = self.compute_objective_function(centroids, u_matrix)
    ...
    def get_centroids_(self):
        ...
        while optimize:
            ...
            centroids = self.compute_centroids(u_matrix)
            self.objective_function.append(self.compute_objective_function(centroids, u_matrix))
            self.get_plot(clusters, centroids)
    ...
    def get_distance_space(self, x):
        ...
        return np.array([[
            get_distance(sample, centroid) for centroid in self.get_clusters_index_cent()[1][0]]
    ...
    def predict_proba_(self, x):
        ...
        for i in range(len(x)):
            ...
            for j in range(self.num_clusters):
                denomenator += np.power(
                    1 / get_distance(self.fit_cent[0][j], x[i]), 2 / (self.fuzzifier - 1))
            for j in range(self.num_clusters):
                uik_new = np.power(1 / get_distance(self.fit_cent[0][j], x[i]),
                                   2 / (self.fuzzifier - 1)) / denomenator
