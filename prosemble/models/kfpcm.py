"""
Implementation of Kernel Fuzzy Possibilistic c-Means Alternative Optimization Algorithm
"""
...
class KFPCM:
    ...
    def __init__(self, data, c, m, num_iter, epsilon, ord, eta, sigma, set_centroids=None,
                 set_U_matrix=None,
                 plot_steps=False):
        self.data = data
        self.num_clusters = c
    ...
    def compute_centroids(self, fuzzy_matrix, centroids, t_matrix):
    ...
    def compute_objective_function(self, centroids, u_matrix, t_matrix):
        objective_function = 2 * np.sum([[
            (1 - self.gaussian_kernel(self.data[i], centroids[j])) *
    ...
    def get_centroids(self):
        ...
        centroids = self.compute_centroids(u_matrix, self._select_prototypes(), t_matrix)
        for num in range(self.num_iter):
            ...
            t_matrix = self.update_fuzzy_matrix(centroids_old, t_matrix)
            centroids = self.compute_centroids(u_matrix, centroids_old, t_matrix)
    ...
    def get_centroids_(self):
        ...
        while optimize:
            ...
            t_matrix = self.update_fuzzy_matrix(centroids, t_matrix)
            centroid_num = centroids
            centroids = self.compute_centroids(u_matrix, centroids, t_matrix)
