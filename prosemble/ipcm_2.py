"""
Implementation of Improved Possibilistic c-Means 2 Alternative Optimization Algorithm
"""
...
class IPCM2:
    ...
    def update_tipicality_matrix(self, centroids, g_matrix, t_matrix):
        ...
        for i in range(len(self.data)):
            for j in range(self.num_clusters):
                exponent = np.exp(-(get_distance_squared(centroids[j], self.data[i]) / g_matrix[j]))
                tik_new = exponent
        ...
        for i in range(len(self.data)):
            ...
            for j in range(self.num_clusters):
                den = \
                    (g_matrix[j] *
                     (1 - (np.exp(-(
                             get_distance_squared(centroids[j], self.data[i]) /
                             g_matrix[j])))))
    ...
    def compute_objective_function_0(self, centroids, t_matrix, fuzzy_matrix):
        ...
        objective_objective = \
            np.sum([[get_distance_squared(self.data[i], centroids[j]) * t_matrix[i][j] *
    ...
    def _distance_(samples, centroid):
        return np.sum(
            [get_distance(sample, centroid) for index, sample in enumerate(samples)]
        )
    ...
    def predict_tipicality_(self, x):
        ...
        for i in range(len(x)):
            for j in range(self.num_clusters):
                exponent = get_distance_squared(self.fit_cent[0][j], self.data[i]) / self.eta[0][j]
