...
class FCM_JAX:
    ...
    def _fit_loop(
        ...
        Old Implementation (NumPy):
            ...
            for num in range(max_iter):
                ...
                u_matrix = update_fuzzy_matrix(centroids_old, u_matrix)
                centroids = compute_centroids(u_matrix)
                if convergence_check(centroids_old, centroids):
                    break
            ```
            Issues:
            - Interpreted Python loop
            - Conditional breaks (hard to optimize)

        New Implementation (JAX):
            ```python
            def scan_fn(state, _):
                return single_iteration(state, X)

            final_state, history = jax.lax.scan(
                scan_fn, initial_state, None, length=max_iter
            )
            ```
            Benefits:
            - Compiled loop (XLA)
            - Fixed number of iterations (predictable)
            - All state updates vectorized
        ...
        # Note: We run all iterations even if converged early
        # This is for JIT compatibility (fixed computation graph)
        final_state, history = jax.lax.scan(
            scan_fn,
            initial_state,
            None,
    ...
    def get_distance_space(self, X: jnp.ndarray) -> jnp.ndarray:
        ...
        X = jnp.asarray(X)
        return euclidean_distance_matrix(X, self.centroids_)
