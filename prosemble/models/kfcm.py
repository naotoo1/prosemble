"""
Implementation of Kernel Fuzzy c-Means Alternative Optimisation Algorithm
"""
...
class KFCM:
    ...
    def __init__(self, data, c, m, num_iter, epsilon,
                 ord, sigma,
                 set_prototypes=None,
                 set_U_matrix=None,
                 plot_steps=False):
    ...
    def update_fuzzy_matrix(self, centroids, u_matrix):
        ...
        for i in range(len(self.data)):
            ...
            for j in range(self.num_clusters):
                denomenator += \
                    np.power(1 / (1 - self.gaussian_kernel(centroids[j], self.data[i])),
                             1 / (self.fuzzifier - 1))
            for j in range(self.num_clusters):
    ...
    def compute_objective_function(self, centroids, u_matrix):
        objective_function = np.sum(
            [[(1 - self.gaussian_kernel(self.data[i], centroids[j])) * 2 *
    ...
    def get_centroids(self):
        ...
        for num in range(self.num_iter):
            ...
            centroids = self.compute_centroids(u_matrix, centroids_old)
    ...
    def get_centroids_(self):
        ...
        while optimize:
            ...
            centroids = self.compute_centroids(u_matrix, centroids)
    ...
    def get_distance_space(self, x):
        ...
        return np.array([[
            euclidean_distance(sample, centroid) for centroid in self.get_clusters_index_cent()[1][0]]
