"""
Implementation of Possibilistic Fuzzy c-Means Alternative Optimization Algorithm
"""
   ...
   """
    ...
    def compute_gamma(self, fuzzy_matrix, centriods):
        ...
        centroid_numerator = [
            [np.multiply(fuzzified_assignments[cluster_index][index],
                         get_distance(sample, centriods[cluster_index])) for
        ...
        for i in range(len(self.data)):
            ...
            for j in range(self.num_clusters):
                denomenator += np.power(
                    1 / get_distance(centroids[j], self.data[i]), 2 / (fuzzifier - 1))
            for j in range(self.num_clusters):
    ...
    def update_tipicality_matrix(self, centroids, g_matrix, t_matrix):
        ...
        for i in range(len(self.data)):
            for j in range(self.num_clusters):
                denomenator = np.power(
                    self.b * (get_distance_squared(centroids[j], self.data[i]) / g_matrix[j]),
                    1 / (self.eta - 1))
    ...
    def compute_objective_function(self, centroids, u_matrix, t_matrix):
        objective_distance = \
            np.sum([[get_distance_squared(self.data[i], centroids[j]) *
    ...
    def get_distance_space(self, x):
        ...
        distance_space = \
            np.array([[get_distance(sample, centroid)
