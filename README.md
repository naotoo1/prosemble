...
Below is a simulation of the technique with the wdbc data set using the ```test_wdbc.py``` based on transfered tranfered pre-trained model prototypes from ```GLVQ```, ```GMLVQ``` and ```CELVQ``` models.
